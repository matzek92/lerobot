...
def _extract_complementary_data(batch: dict[str, Any]) -> dict[str, Any]:
    ...
    This includes padding flags, task description, indices, and any additional
    task-level data keys (e.g. ``task.goal_image`` for goal-conditioned policies).
    ...
    # Pass through any additional ``task.*`` sub-keys (e.g. ``task.goal_image``).
    task_sub_keys = {k: v for k, v in batch.items() if k.startswith("task.")}
