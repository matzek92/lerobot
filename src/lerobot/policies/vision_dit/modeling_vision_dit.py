...
"""Vision-only Diffusion Transformer (DiT) Policy

DiT-based diffusion / flow-matching policy conditioned on robot state and images only.
The image encoder is a freely configurable torchvision CNN backbone (ResNet18 by default).
"""
...
from .configuration_vision_dit import VisionDiTConfig
...
class VisionDiTPolicy(PreTrainedPolicy):
    config_class = VisionDiTConfig
    name = "vision_dit"

    def __init__(self, config: VisionDiTConfig, **kwargs):
        require_package("diffusers", extra="vision_dit")
...
def _build_cnn_backbone(name: str, weights: str | None) -> tuple[nn.Module, int]:
    ...
    Returns the backbone module (producing a flat ``(B, embed_dim)`` feature vector) and
    the embedding dimension.
    """
    model = torchvision.models.get_model(name, weights=weights)

    # Most torchvision classifiers expose either ``fc`` (resnet family, regnet, googlenet,
    # inception) or ``classifier`` (efficientnet, convnext, mobilenet, vgg, mnasnet,
    # densenet, shufflenet, squeezenet). We replace the head with Identity so the model
    # returns the pooled+flattened pre-classifier features.
...
class CNNVisionEncoder(nn.Module):
    """Configurable torchvision CNN encoder producing a flat per-image feature vector."""

    def __init__(self, backbone_name: str, pretrained_weights: str | None):
        super().__init__()
        self.backbone_name = backbone_name
        self.backbone, self.embed_dim = _build_cnn_backbone(backbone_name, pretrained_weights)
...
class ObservationEncoder(nn.Module):
    """Encodes images + robot state into a flat conditioning vector for the DiT."""

    def __init__(self, config: VisionDiTConfig):
        super().__init__()
        self.config = config
                ...
                self.vision_encoders = nn.ModuleList(
                    [
                        CNNVisionEncoder(
                            ...
                            pretrained_weights=config.pretrained_backbone_weights,
                        )
