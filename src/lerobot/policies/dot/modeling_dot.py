...
"""DOT policy adapted to the current LeRobot policy API."""
...
class DOT(nn.Module):
    def __init__(self, config: DOTConfig):
        ...
        # Use one backbone for all cameras and project each image to model dim.
        ...
        decoder_pos = create_sinusoidal_pos_embedding(
            config.train_horizon + config.lookback_obs_steps,
            config.dim_model,
        )
...
class DOTPolicy(PreTrainedPolicy):
    ...
    def _update_observation_buffers(self, buffer_name: str, observation: Tensor) -> Tensor:
        repeat_dims = (1, self.config.lookback_obs_steps + 1, *([1] * (observation.ndim - 1)))

        if buffer_name not in self._input_buffers:
            self._input_buffers[buffer_name] = observation.unsqueeze(1).repeat(*repeat_dims)
        else:
    ...
    def select_action(self, batch: dict[str, Tensor], **kwargs) -> Tensor:
        ...
        if self.step % self.predict_every_n == 0:
            self.last_action = self.model(batch)[:, -self.config.inference_horizon :]
        else:
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        keys_to_trim = list(self.model.obs_mapping.values()) + list(self.image_names) + [ACTION, "action_is_pad"]

        for key in keys_to_trim:
            if key not in batch or key == OBS_IMAGES:
                continue
            batch[key] = torch.cat(
                [
                    batch[key][:, lookback_ind : lookback_ind + 1],
                    batch[key][:, 2 * self.config.lookback_aug + 1 :],
                ],
                dim=1,
            )
        ...
        if len(self.image_names) > 0:
            ...
            for key in self.image_names:
                bs, n_obs, c, h, w = batch[key].shape
                batch[key] = batch[key].view(bs * n_obs, c, h, w)
                batch[key] = crop_transform(self.resize_transform(batch[key]))
                batch[key] = batch[key].view(bs, n_obs, c, *batch[key].shape[-2:])

            batch[OBS_IMAGES] = torch.stack([batch[k] for k in self.image_names], dim=2).flatten(1, 2)

        if self.state_noise is not None:
            for key in self.model.obs_mapping.values():
                if key != OBS_IMAGES and key in batch:
                    batch[key] += (torch.rand_like(batch[key]) * 2 - 1) * self.state_noise
