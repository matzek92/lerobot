...
### Optional previous-remaining trajectory conditioning

Set `--policy.use_prev_remaining_traj=true` to enable an ACT flavor that conditions decoding on the remaining
trajectory from the previous prediction chunk. The first step (and steps after `policy.reset()`) uses an
all-zero initializer for this extra input.
